...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- Brave is a privacy-focused third engine with good technical indexing.\n"
...
async def web_search(
    ...
    Args:
        ...
        engines: Engines to query (default: configured engines). Options: "bing", "duckduckgo", "brave", "google" (deprecated).
...
async def search_brave(
    ...
    """Search Brave Search directly via its HTML endpoint.

    Privacy-focused, no JavaScript rendering required.
    Good technical-content indexing and minimal anti-bot measures.
...
def _build_dual_transport_app():
    ...
    async def health_endpoint(request: Request) -> Response:
        from evoscry.cache import _MAX_ENTRIES, cache_size
        from evoscry.circuit_breaker import all_breakers
        ...
        for name, cb in all_breakers().items():
            ...
            engines_health[name] = {
                "state": s["state"],
                "searches": snap["searches"]["by_engine"].get(name, 0),
                "errors": snap["searches"]["errors_by_engine"].get(name, 0),
                "last_success": snap["searches"]["last_success_by_engine"].get(name),
                "last_failure": snap["searches"]["last_failure_by_engine"].get(name),
            }
        ...
        body = {
            ...
            "ai": {
                "copilot_configured": config.copilot_token is not None or config.copilot_refresh_token is not None,
                "ollama_configured": config.local_model_url is not None,
                **snap["ai"],
            },
            "config": {
    ...
    async def readiness_endpoint(request: Request) -> Response:
        from evoscry.circuit_breaker import CircuitState, all_breakers
        from evoscry.config import load_config
        from evoscry.metrics import metrics
        ...
        # Ready if at least one configured engine is not OPEN
        if breakers:
            at_least_one_healthy = any(
                cb.state != CircuitState.OPEN
                for name, cb in breakers.items()
                if name in config.search_engines
            )
        else:
            # No searches yet — assume ready
            at_least_one_healthy = True

        ready = at_least_one_healthy
        body = {
            "ready": ready,
            "checks": {
                "at_least_one_engine_healthy": at_least_one_healthy,
                "cache_initialized": True,
            },
        }
        status = 200 if ready else 503
        return Response(json.dumps(body), media_type="application/json", status_code=status)
